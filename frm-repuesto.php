   ...
   <body onload="cpestana41(6);">
      ...
      <div id="pestanas4-1">
         <ul id=lista>
            ...
            <li id="p4-1-4"><a href='javascript:cpestana41(4);'>Detal remision</a></li>
      ...
      <div id="cpestana4-1-6">
          ...
          // completar con la info de la bd el inventario de maquinas
                        $sig_id_remision=generarid("remisionrpto");
                        $fremision=date("Y-m-d");
                        $observacion="";
                        $autorizacion="";
                        $confrecibido="";
                        $fcreacion=date("Y-m-d H:i:s");
                        $zona="";
                        $id_detremision=generarid("detalle_remisionrpto");
          //}else{header("location:../login.php");}
          escribir("se imprimen variables para remision ");
          ?>
          <style>
          input[type=number]{
              width:50px;
          ...
          <form id="frm-crear-repuesto" name="formulario-repuesto" method="post" action="centalop.php" enctype="multipart/form-data">
            ...
            <table id="main-rpt-crear" >
                <tr>
                    <td>
                        ...
                        <tr class="encabezado"><td colspan="2"><img src="images/jerencrem.fw.png" /></td><td colspan="12">Nuevo Repuesto</td></tr>
                        <tr class="linea"><td colspan="14">
                        <input name="opr" type="hidden"  value="crear-repuesto"/>
                        </td></tr>
                        <tr  class="titulo"><td colspan="15">Crear Repuesto</td></tr>
                        <tr class="encabezados"><td>iddb</td><td>Refaccess</td><td>Nombre</td><td>Cantidad</td><td>salida</td><td>Entrada</td><td>Observacion</td><td>Fcreacion</td></tr>
                        <tr>
                        <td><input name="iddb" type="number"  value="<?php echo generarid('repuesto'); ?>" disabled /></td>
                        <td><input name="refaccess" type="text" value=""   required/></td>
                        <td><input name="nombre" type="text" value=""  required list="lstproducto"/></td>
                        <td><input name="cantidad" type="number"   value="0" required  /></td>
                        <td><input name="salida" type="number"   value="0"  required/></td>
                        <td><input name="entrada" type="number" value="0"  required/></td>
                        <td><input name="observacion" type="text" value=""  required/></td>
                        <td><input name="fcreacion" type="date" value="<?php echo $fremision=date("Y-m-d"); ?>"  required /></td>
                        </tr>
          ...
          <script>
                       function fnaddrepuesto(){
							...
							$('#frm-crear-repuesto').submit(function(event){
							...
							if(confirm("Esta Seguro de insertar")){}else{return false;}
							  ...
							  $.ajax({
								  ...
								  success     : function(data, textStatus, jqXHR){
									...
									if(data.error==0){
									   alert(data.msj + "idrep="+data.idrep);	
										}else{
										alert(data.msj);
									}
								  }
      ...
      <div id="cpestana4-1-1">
         <div id="crearcliente">
            ...
            <script>
               var tablac="repuesto"//tabla a la que se realizaran las consultas 
               ...
               clicreados(1);
               //funcion para cargar la remision en el iframe de abajo al dar click en el boton de la remision correspondiente
               function cargaminif(idrem){$("#framremision").attr('src','inforemision.php?idrem='+idrem);}
            ...
            <div id="minremision">
               <iframe id="framremision" dir=""></iframe>
            </div>
         </div>
      </div>
      ...
      <table class="tregistro"  border="0">
         <thead>
            ...
            <tr>
               <th colspan="5"><input id="txttabla" type="text" required  list="listtablas" onBlur="traercampos()" value="remision"  hidden="true"/></label></th>
            </tr>
            <tr>
               <th colspan="5">
      ...
      <script>
         ...
         //funciones para cargar el detalle de la remision
         function cargaminif2p(idrem){$("#framremision2p").attr('src','inforemision.php?idrem='+idrem);}
         function cargarremision2p(idrem){
         	...
         	$.ajax({
         		...
         		error: function(){curnormal();},
         			success: function(data){
         				...
         				strcp+="<td><div class='btn' onclick='cargarremision2p("+data.elementos[el][0]+")'>"+"Verrem"+"</div></td>";
         				strcp+="</tr>";
         				} 
         				strcp+="<tfoot>";
         				strcp+="<tr><td colspan='"+(numcampos+1)+"'>";
      ...
      <!--
         ...
         <div id="minremision2p">
         <iframe id="framremision2p" dir=""></iframe>
         </div>
         -->                                                
      </div>
      </div>
            ...
            <div id="cpestana4-1-4">
               <table border="1" style="margin:auto;background-color:rgba(255,255,255,1);text-align:center;">
                  <tr>
                     <th colspan="2">Consultar Detalle de remision</th>
                  </tr>
                  <tr>
                     <td>Ingrese el id de la remision</td>
                  ...
                  <tr>
                     <td colspan="2">
                        <script>
                           ...
                           $("#framremision4p").attr('src','inforemision.php?idrem='+idrem);
                           }
                           function cargarremision4p(idrem){
                        ...
                        <div id="minremision4p">
                           <iframe id="framremision4p" dir=""></iframe>
                        </div>
                     </td>
                  </tr>
               </table>
