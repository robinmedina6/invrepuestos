...
if(isset($_POST['nda'])){
	...
	$sql="SELECT serial, id FROM maquina WHERE id >= $mi AND id<=$mf";
	...
	while($rowmaquina=mysqli_fetch_array($rtamaquinas)){
		...
		$sql="SELECT * FROM logueo AS l WHERE l.id_maquina=".$rowmaquina['id']. " AND l.fecha='$fecha'  ORDER BY l.fecha DESC LIMIT 1";
              $rtalogueo=ejecutarsql($sql);
			  if($rtalogueo){
			  $numrows=mysqli_num_rows($rtalogueo);
			  if($numrows>0){
		...
		var url="http://www.globalsa.com.co/prueba/produccion/reportes/porterminal.php";
		  var pagi=1;
		   var fecha=flogueo;
		  ...
		  $.ajax({
			  ...
			  success: function(data){
				  ...
				  if(data.indexOf("ENCONTRARON")==-1){//se encontro resultados
				  	if(data.indexOf("<?php echo $serial; ?>")>0){//se encontro resultados
					...
					}else{$("#avc").html("sin resultados de mauin");}
        ...
        <table border="1">
          <tr>
           ...
           <tr>
			...
			<div class="btn" onclick="redir('pruebas.php')">Actpruebas</div>
